      ...
      Promise.all([
                ...
                let pert1Config = {
                  ...
                  let pert3Config = {
                    ...
                    tooltipConfig: [
                      {label: "ML210 Dose (µM)", field: "x"},
                      {label: "Ferrostatin-1 (µM)", field: "y"},
                      {label: "Cell lines killed (Viability < 0.3)", field: "value"}
                    ]
                  }
               ...
               return groups.map((d) => {
                  return d[1].map((e) => { 
                     ...
                     //    c: ((e[1].filter(f => f.value === true).length)/e[1].length)*100,
                        c: e[1].filter(f => f.value === true).length + 1, // add 1 for log scale
              ...
              let groups = d3.groups(data, d => d.x)
               ...
               return groups.map((d,i) => {
                      return {
                        x: d[0],
                        y: 0,
                        c: (+d[1].filter(f => f.value === true).length)+1, // add 1 for log scale
